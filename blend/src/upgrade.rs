...
fn upgrade_proto(ctx: &Context) -> anyhow::Result<()> {
    ...
    run_cmd(ctx, "proto", &["outdated", "--yes", "--update"], &proto_dir)?;
...
///
/// `sync_fn` is called at the end of a full upgrade to sync dotfiles,
/// avoiding circular module dependency.
pub fn cmd_upgrade<F>(ctx: &Context, step: &Option<UpgradeStep>, sync_fn: F) -> anyhow::Result<()>
where
    F: FnOnce(&Context) -> anyhow::Result<()>,
{
    ...
    type StepEntry = (&'static str, fn(&Context) -> anyhow::Result<()>);
    ...
    for (name, step_fn) in &steps {
        if let Err(e) = step_fn(ctx) {
            log::error(&format!("{name} upgrade failed: {e}"));
    ...
    if let Err(e) = sync_fn(ctx) {
        log::error(&format!("Dotfile sync failed: {e}"));
