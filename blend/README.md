...
│   ├── status.rs        package state table (parallel via rayon)
│   └── table.rs         HTML table for README
│
├── compose.rs           build_package: evaluate .ncl → BuildResult
├── sync.rs              SyncMode/Action, Prompter trait, pull_from_*
├── upgrade.rs           system upgrade orchestration (brew/pacman/proto)
│
├── nickel.rs            re-exports schema types + NickelEvaluator
├── nickel/
│   ├── schema.rs        FileEntry, Format, OrderPackage, WhenCondition
...
│   ├── delimited.rs     space/equal-delimited variants
...
tests/
├── sync_e2e.rs          end-to-end CLI tests (29 scenarios)
...
| How `.ncl` is rewritten on pull | `nickel/structure_map.rs`, `nickel/ast_utils.rs` |
...
- **End-to-end** tests in `tests/sync_e2e.rs` drive the compiled binary
  against tempdir fixtures — push/pull/view/status flows, symlink
  redeploy, per-key interactive sync.
