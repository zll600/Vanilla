...
echo ""
if [[ "$os" == "Darwin" ]]; then
  echo "  3. Lock shell rc files (prevent external modification):"
  echo "     chflags uimmutable ~/.config/elvish/rc.elv ~/.zshrc ~/.zshenv ~/.bashrc ~/.profile ~/.hushlogin"
  echo ""
  echo "  4. Apply macOS system preferences:"
  echo "     ./macos_config.sh"
  echo ""
  echo "  5. Configure Raycast (https://manual.raycast.com/hotkey)"
